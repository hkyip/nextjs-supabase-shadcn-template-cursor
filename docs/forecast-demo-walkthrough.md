...
- Open **Forecast** in another (or the same tab): `/forecast`

Both use the same **demo session** (shared `ProductionProvider` in the demo layout), so numbers on `/forecast` can line up with **What to Cook** when you expand **Live bridge** on the forecast page.

## Script

1. **Forecast (`/forecast`)** — “This is the **plan from the clock**, not from last sale.”
...
3. **Production (`/production`)** — “Here’s where forecast meets inventory.”
   ...
   - Short line: cook target combines **forecast − pipeline + queue + lane** (see explainer card on `/forecast`).
   - Mention the link line at the top: **Demand curve & today’s time buckets → /forecast**.
